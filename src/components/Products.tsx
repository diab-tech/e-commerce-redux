...
const Products: React.FC = () => {
  ...
  if (isLoading) return <div className="w-full text-center py-8">Loading...</div>;
  if (queryError) return <div className="w-full text-center py-8 text-red-500">Error: {queryError?.message}</div>;
  if (!data || !Array.isArray(data) || data.length === 0) return <div className="w-full text-center py-8">No products available</div>;
